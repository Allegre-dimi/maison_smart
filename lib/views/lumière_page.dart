...
class LampeScreen extends StatefulWidget {
  ...
  @override
  State<LampeScreen> createState() => _LampeScreenState();
}
...
class _LampeScreenState extends State<LampeScreen>
  ...
  Module? _module;
  String? _error;
  bool lampeManuelle = false;
  bool horaireActive = false;
  bool get lampeActive => lampeManuelle || horaireActive;
  ...
  void initState() {
    ...
    _load();
    refreshTimer = Timer.periodic(const Duration(seconds: 10), (_) => _load());
    _startMonitoring();
  }
  ...
  Future<void> _load() async {
    try {
      final m =
          await ModuleService().getModule(widget.moduleId, type: 'eclairage');
      if (mounted && m != null) {
        setState(() {
          _module = m;
          lampeManuelle = m.etat;
        });
      }
    } on ApiException catch (e) {
      if (mounted) setState(() => _error = e.message);
    }
    _refreshHoraireStatus();
  }
    ...
    timer = Timer.periodic(const Duration(seconds: 2), (t) {
      if (lampeActive) {
        setState(() {
          ...
          consommationActuelle += mesureCourant * 0.001;
  ...
  Future<void> _toggleManuel(bool newState) async {
    try {
      final updated = await ModuleService()
          .setEtat(widget.moduleId, newState, type: 'eclairage');
      if (mounted) {
        setState(() {
          _module = updated;
          lampeManuelle = updated.etat;
        });
      }
    } on ApiException catch (e) {
  ...
  Widget build(BuildContext context) {
    if (_module == null) {
      return Scaffold(
        appBar: AppBar(title: Text("Lampe - ${widget.pieceName}")),
        body: Center(
          child: _error == null
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("Lampe - ${widget.pieceName}"),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    color: lampeActive ? Colors.deepPurple : Colors.grey[300],
                    boxShadow: [
                      if (lampeActive)
                        BoxShadow(
                  ...
                  child: Icon(
                    lampeActive ? Icons.lightbulb : Icons.lightbulb_outline,
                    color: lampeActive ? Colors.white : Colors.black54,
                    size: 70,
                  ),
                ),
              ),
              ...
              Text(
                lampeActive ? "Lampe allumée" : "Lampe éteinte",
                style: TextStyle(
                  color: lampeActive ? Colors.amber[700] : Colors.black45,
